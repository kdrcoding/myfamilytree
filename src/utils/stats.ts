...
export function computeStats(people: FamilyPerson[]): FamilyStats {
  ...
  const countries = [...new Set(people.map((p) => p.country?.trim()).filter(Boolean))] as string[];
  ...
  return {
    ...
    mostChildren,
    mostDescendants,
    perGeneration: [...genCounts.entries()]
      .sort((a, b) => a[0] - b[0])
    ...
    genderSplit: [
      { label: 'Men', count: people.filter((p) => p.gender === 'male').length },
      { label: 'Women', count: people.filter((p) => p.gender === 'female').length },
      { label: 'Unspecified', count: people.filter((p) => p.gender === 'unspecified').length },
    ],
  };
}
