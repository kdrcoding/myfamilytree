  ...
  return (
    <div className="app-shell relative min-h-dvh overflow-hidden">
      ...
      <div className="relative z-10 mx-auto flex min-h-dvh max-w-lg flex-col px-5 pb-10 pt-8 sm:pt-12">
        ...
        {state === 'list' && (
          <>
            <p className="mt-8 text-center text-sm font-medium text-teal-950/90">
                ...
                return (
                  <li key={person.id}>
                    <Link
                      ...
                      onClick={keepPass}
                      className="flex items-center gap-3 rounded-2xl border border-teal-900/15 bg-[rgb(200_222_214/0.88)] px-3 py-2.5 shadow-sm backdrop-blur transition hover:bg-[rgb(214_230_224/0.95)]"
                    >
                      {photo ? (
                        <img
                          src={photo}
                      ...
                      <div className="min-w-0 flex-1 text-left">
                        ...
                        {person.whoLine && (
                          <p className="truncate text-xs text-stone-600">{prettyLabel(person.whoLine)}</p>
                        )}
                      </div>
                      <UserRoundPen className="h-4 w-4 shrink-0 text-teal-800" aria-hidden />
                    </Link>
                  </li>
                );
              })}
            ...
            <Link
              to={fillHref}
              onClick={keepPass}
              className="btn-primary mt-6 inline-flex min-h-12 w-full items-center justify-center gap-2"
            >
              <CalendarDays className="h-5 w-5" aria-hidden />
              {t('dates.fillAll')}
            </Link>
            <p className="mt-3 text-center text-xs leading-relaxed text-stone-600">{t('dates.fillHint')}</p>
          </>
        )}
